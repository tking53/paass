...
void VandleProcessor::rootVWaveInit(VWave &strutName) { //Zeros the WaveForm
// Structures
    fill(strutName.trace, strutName.trace + 131, 0);
    strutName.baseline = -999;
    strutName.baselineSTD = -999;
    strutName.maxLoc = -999;
    strutName.amp = -999;
    strutName.phase = -999999;
    strutName.LowResTime = -999;
    strutName.ID = -999;
}
...
VandleProcessor::VandleProcessor(const std::vector<std::string> &typeList,
    ...
    string rootname3 = hisfilename + "-Vandle-Vwave.root";
    rootFName3_ = new TFile(rootname3.c_str(), "RECREATE");
    Wave = new TTree("Wave", "Tree for Waveform Analyzer Debugging");
    VwaveBranch = Wave->Branch("Vwave", &Vwave,
                               "trace[131]/D:LowResTime/D:ID:baseline:"
                                       "baselineSTD:maxLoc:amp:qdc:phase");
    ...
    for (BarMap::iterator it = bars_.begin(); it != bars_.end(); it++) {
        ...
        for (BarMap::iterator itStart = barStarts_.begin();
            ...
            for (vector<unsigned int>::const_iterator itTL = bar.GetLeftSide().GetTrace().begin();
                ...
                Vwave.trace[itTVl]=(*itTL);
                itTVl++;
            };

            Vwave.LowResTime=bar.GetLeftSide().GetTime();
            Vwave.ID=bar.GetLeftSide().GetId();
            Vwave.baseline=bar.GetLeftSide().GetAveBaseline();
            Vwave.baselineSTD=bar.GetLeftSide().GetStdDevBaseline();
            Vwave.maxLoc=bar.GetLeftSide().GetMaximumPosition();
            Vwave.amp=bar.GetLeftSide().GetMaximumValue();
            Vwave.qdc=bar.GetLeftSide().GetTraceQdc();
            Vwave.phase=bar.GetLeftSide().GetPhaseInNs();

            Wave->Fill();


            Vwave.LowResTime=bar.GetRightSide().GetTime();
            Vwave.phase=bar.GetRightSide().GetPhaseInNs();
            Vwave.qdc=bar.GetRightSide().GetTraceQdc();
            Vwave.amp=bar.GetRightSide().GetMaximumValue();
            Vwave.maxLoc=bar.GetRightSide().GetMaximumPosition();
            Vwave.baseline=bar.GetRightSide().GetAveBaseline();
            Vwave.baselineSTD=bar.GetRightSide().GetStdDevBaseline();
            Vwave.ID=bar.GetRightSide().GetId();
            int itTVr=0;
            for (vector<unsigned int>::const_iterator itTR = bar.GetRightSide().GetTrace().begin();
                 itTR != bar.GetRightSide().GetTrace().end(); itTR++) {
                Vwave.trace[itTVr]=(*itTR);
                itTVr++;
            };
