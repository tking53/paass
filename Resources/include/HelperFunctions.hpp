...
static const pair<double, vector<double> > CalculatePoly2(
    ...
    if (data.size() < 3)
        throw range_error(
            "Polynomial::CalculatePoly2 - The data vector "
            "had the wrong size : " +
            std::to_string(data.size()));
...
static const pair<double, vector<double> > CalculatePoly3(
    ...
    if (data.size() < 4)
        throw range_error(
            "Polynomial::CalculatePoly3 - The data vector "
            "had the wrong size : " +
            std::to_string(data.size()));
    ...
    double p0 = (double)((data[x1[0]] * (x1[1] * (x2[2] * x3[3] - x2[3] * x3[2]) -
                                         x1[2] * (x2[1] * x3[3] - x2[3] * x3[1]) +
                                         x1[3] * (x2[1] * x3[2] - x2[2] * x3[1])) -
    ...
    double p2 = (double)(((x1[1] * (data[x1[2]] * x3[3] - data[x1[3]] * x3[2]) -
    ...
    double p3 = (double)(((x1[1] * (x2[2] * data[x1[3]] - x2[3] * data[x1[2]]) -
...
inline double CalculateIntegral(const vector<T> &data) {
    if (data.size() < 2)
        throw range_error(
            "Statistical::CalculateIntegral - The data "
...
inline pair<double, double> CalculateBaseline(
    ...
    if (data.size() == 0)
        throw range_error(
            "TraceFunctions::ComputeBaseline - Data vector "
            "sized 0");

    if (range.second < range.first)
        throw range_error(
            "TraceFunctions::ComputeBaseline - Bad range : "
            "High > Low");
    ...
    if (range.second - range.first < minimum_baseline_length)
        throw range_error(
            "TraceFunctions::ComputeBaseline - The range "
...
inline pair<double, vector<double> > ExtrapolateMaximum(
    ...
    if (data.size() < 4)
        throw range_error(
            "TraceFunctions::ExtrapolateMaximum - "
...
inline pair<unsigned int, double> FindMaximum(
    ...
    if (data.size() == 0)
        throw range_error(
            "TraceFunctions::FindMaximum - The data was of "
            "size 0.");
...
inline unsigned int FindLeadingEdge(const vector<T> &data,
    ...
    if (fraction <= 0)
        throw range_error(
            "TraceFunctions::FindLeadingEdge - The "
            "threshold was below zero.");
    if (data.size() < minimum_baseline_length)
        throw range_error(
            "TraceFunctions::FindLeadingEdge - The data "
...
inline double CalculateQdc(const vector<T> &data,
    ...
    if (data.size() == 0)
        throw range_error(
            "TraceFunctions::CalculateQdc - The size of "
...
inline double CalculateTailRatio(const vector<T> &data,
    ...
    if (qdc == 0)
        throw range_error(
            "TraceFunctions::CalculateTailRatio - The QDC "
...
/// other than seconds
/// @param[in] value : The value that we want to convert that has units
/// other than seconds
...
/// Function that will calculate nano seconds from a value that has time units
/// other than nano seconds
/// @param[in] value : The value that we want to convert that has units
/// other than nano seconds
/// @param[in] units : The units that the value has
/// @return The value with units of nano seconds.
inline double ConvertNanoSecondsWithPrefix(const double &value, const std::string &units) {
    double val = value;
    if (units == "s") {
        val *= 1e9;
    }else if (units == "ms"){
        ...
        throw std::invalid_argument("HelperFunctions::ConvertNanoSecondsWithPrefix : Unknown units " + units);
    }
    return val;
}
}  // namespace Conversions
